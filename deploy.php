...
$commands = [
    ...
    // Mark vendor git packages as safe to avoid 'dubious ownership' fatal during Composer operations
    ...
    // Install production dependencies without dev and without interaction
    "cd /d " . escapeshellarg($projectPath) . " && composer install --no-dev --no-interaction --optimize-autoloader",
    // Regenerate optimized autoloads to ensure removed dev packages are not referenced
    "cd /d " . escapeshellarg($projectPath) . " && composer dump-autoload -o",
